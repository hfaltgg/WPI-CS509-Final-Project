...
public class SearchDoubleAdapter extends BaseAdapter{
    private List<Pair<Flight, Flight>> mDataset;
    private SeatClass mSeatClass;
    private TripType mTripType;
    private StopOvers mStopOvers;
    private Integer mNumber;

    public SearchDoubleAdapter(List<Pair<Flight, Flight>> mDataset, SeatClass mSeatClass, TripType mTripType, StopOvers mStopOvers, Integer mNumber) {
    ...
    public void setDateset(List<Pair<Flight, Flight>> mDataset) {
        this.mDataset = mDataset;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.mDepAirportCode.setText(mDataset.get(position).first.getmDepAirportCode() + "");
        holder.mArrAirportCode.setText(mDataset.get(position).second.getmArrAirportCode() + "");
        DateFormat dateFormat = new SimpleDateFormat("MMM/dd/yyyy");
        holder.mDepTime.setText(dateFormat.format(mDataset.get(position).first.getmDepTime()) + "");
        holder.mArrTime.setText(dateFormat.format(mDataset.get(position).second.getmArrTime()) + "");
        holder.mFlightTime.setText((mDataset.get(position).first.getmFlightTime() + mDataset.get(position).second.getmFlightTime()) + " min");

        DecimalFormat df2 = new DecimalFormat(".##");
        if (mSeatClass == SeatClass.COACHCLASS) {
            holder.mPrice.setText("$ " + df2.format(mDataset.get(position).first.getmCoachPrice() * mNumber + mDataset.get(position).second.getmCoachPrice() * mNumber));
        } else {
            holder.mPrice.setText("$ " + df2.format(mDataset.get(position).first.getmFirstPrice() * mNumber + mDataset.get(position).second.getmFirstPrice() * mNumber));
        }
